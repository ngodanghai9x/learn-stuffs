const envPath =
    process.env.NODE_ENV === 'development'
        ...
        : require('path').join(process.resourcesPath, 'app.asar/.env');
    ...
    socket.on('joinRoom', ({ type, roomId, password } = {}) => {
        ...
        if (type === 'client') {
            ...
            socket.on('keyTap', (key) => {
                if (
                    !new RegExp(/xxx[\x00-\x7F]+xxx/).test(key) &&
...
app.use(
    ...
    expressDocs(app, {
      ...
      `,
            ...
            customCode:
                fs.readFileSync('./data/posthog.js', 'utf8') ||
                (() => {}).toString() +
                    `
...
app.get('/api/v1/feedback/get', (req, res) => {
    readDatabase('feedback')
        .then(({ feedback = [] }) => {
            res.status(200).json({
                err: null,
                feedback: feedback.reduce(
                    (accumulator, { rating }) =>
                        accumulator.map((starCount, index) => starCount + (index + 1 === rating)),
                    [0, 0, 0, 0, 0],
                ),
            });
        })
...
app.post('/api/v1/feedback/send', (req, res) => {
    ...
    if (typeof req.body?.rating !== 'number' || req.body?.rating < 1 || req.body?.rating > 5)
        return res.status(422).json({ err: 'Invalid rating' });
    readDatabase('feedback')
        .then(({ feedback = [] }) => {
...
app.get('/api/v1/apps/get', (req, res) => {
    readDatabase('apps')
        .then(({ apps = {} }) => {
            res.status(200).json({
                err: null,
                apps: Object.entries(apps).reduce(
                    (accumulator, [appId, { iconExtension, name, description, verified }]) => ({
                        ...accumulator,
                        ...{
                            [appId]: {
                                iconExtension,
...
app.post('/api/v1/newsletter/register', (req, res) => {
    ...
    readDatabase('newsletter')
        .then(({ newsletter = [] }) => {
            if (newsletter.includes(req.body?.email))
                return res.status(409).json({ err: 'Email address is already registered' });
            updateDatabase('newsletter', [...newsletter, ...[req.body?.email]])
                .then(() => {
                    res.status(200).json({ err: null });
                })
                .catch(({ err }) => {
...
app.post('/api/v1/newsletter/send', (req, res) => {
    ...
    if (req.cookies?.adminPassword !== process.env.ADMIN_PASSWORD)
        return res.status(418).json({ err: "I'm a teapot" });
    if (!req.body?.subject) return res.status(404).json({ err: 'Missing subject' });
    if (typeof req.body?.subject !== 'string' || req.body?.subject.length < 1)
        return res.status(422).json({ err: 'Invalid subject' });
    if (!req.body?.type) return res.status(404).json({ err: 'Missing type' });
    if (!['text', 'html'].includes(req.body?.type))
        return res.status(422).json({ err: 'Invalid type' });
    if (!req.body?.content) return res.status(404).json({ err: 'Missing content' });
    if (typeof req.body?.content !== 'string' || req.body?.content.length < 1)
        return res.status(422).json({ err: 'Invalid content' });
    readDatabase('newsletter')
        .then(({ newsletter = [] }) => {
            ...
            Promise.all(
                Array.from(Array(Math.ceil(newsletter.length / 10))).map((_, index) => {
                    return Promise.all(
                        newsletter.slice(index * 10, (index + 1) * 10).map((email) => {
                                ...
                                switch (step) {
                                    ...
                                    case 1:
                                        if (data.toString().startsWith('334')) {
                                            client.write(
                                                Buffer.from(process.env.EMAIL_ADDRESS).toString(
                                                    'base64',
                                                ) + '\r\n',
                                            );
                                            step++;
                                    ...
                                    case 2:
                                        if (data.toString().startsWith('334')) {
                                            client.write(
                                                Buffer.from(process.env.EMAIL_PASSWORD).toString(
                                                    'base64',
                                                ) + '\r\n',
                                            );
                                            step++;
...
app.get('/api/v1/admin/verify', (req, res) => {
    ...
    if (!req.cookies?.adminPassword)
        return res.status(404).json({ err: 'Missing password', valid: false });
    res.status(200).json({
        err: null,
