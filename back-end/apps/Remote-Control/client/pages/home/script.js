...
window.process = {
    env: fs
        .readFileSync(path.join(parent.process.resourcesPath, 'app.asar/.env'), 'utf8')
        .split('\n')
        .filter((line) => !line.startsWith('#') && line.split('=').length > 1)
...
document.getElementById('reviewEmbed').src =
    (Object.keys(
        JSON.parse(
            fs.readFileSync(path.join(parent.process.resourcesPath, 'customServer.json'), 'utf8'),
        ),
    ).length
        ? JSON.parse(
              fs.readFileSync(path.join(parent.process.resourcesPath, 'customServer.json'), 'utf8'),
          ...
          (JSON.parse(
              ...
              ? ':' +
                JSON.parse(
                    fs.readFileSync(
                        path.join(parent.process.resourcesPath, 'customServer.json'),
                        'utf8',
                    ),
                ).resourcePort
              : '')
        : process.env.DEFAULT_RESOURCE_SERVER_PROTOCOL +
          '//' +
          process.env.DEFAULT_RESOURCE_SERVER_HOSTNAME +
          (process.env.DEFAULT_RESOURCE_SERVER_PORT
              ? ':' + process.env.DEFAULT_RESOURCE_SERVER_PORT
              : '')) + '/reviews';
...
Array.from(document.getElementById('actionContainer').children).forEach((actionElement, index) => {
    actionElement.addEventListener('click', () => {
        parent.document.getElementById('pageEmbed').src =
            '../' + (index ? 'scripts' : 'monitor') + '/index.html';
        Array.from(parent.document.getElementById('menuBar').children).forEach(
            (unhighlightingMenuBar) => {
        ...
        parent.document.getElementById('menuBar').children[0].children[
            2 + index
        ].style.backgroundColor = '#0c7cb7';
        parent.document
            .getElementById('menuBar')
            .children[0].children[2 + index].classList.remove('menuBarItem');
    });
});

document.getElementById('connectionDetailContainer').children[0].children[1].innerText = JSON.parse(
    localStorage.getItem('loginDetails'),
)[0]
    .split('')
    .map((character, index) => ((index + 1) % 3 === 0 ? character + ' ' : character))
    .join('')
    .substring(
        0,
...
document.getElementById('connectionDetailContainer').children[2].children[1].innerText = JSON.parse(
    localStorage.getItem('loginDetails'),
)[1];
document.getElementById('connectionActionContainer').children[3].addEventListener('click', () => {
    ...
    document.getElementById('connectionActionContainer').children[1].value = '';
    document.getElementById('connectionActionContainer').children[2].value = '';
    let remoteControlWindow = window.open(
        '../remoteControl/index.html',
        '_blank',
        'title=Remote Control,autoHideMenuBar=true,nodeIntegration=true,contextIsolation=false',
    );
...
document.getElementById('regenerateConnectionIcon').addEventListener('click', () => {
    ...
    console.log(
        `🚀 ~ document.getElementById ~ localStorage.getItem("loginDetails"):`,
        localStorage.getItem('loginDetails'),
    );
    ...
    document.getElementById('connectionDetailContainer').children[2].children[1].innerText =
