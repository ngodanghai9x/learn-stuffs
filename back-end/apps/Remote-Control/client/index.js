...
//   }
// }), {}));
if (!require('fs').readdirSync(process.resourcesPath).includes('autoLaunchType.txt'))
    require('fs').writeFileSync(
        require('path').join(process.resourcesPath, 'autoLaunchType.txt'),
        'foreground',
    );
if (!require('fs').readdirSync(process.resourcesPath).includes('customServer.json'))
    require('fs').writeFileSync(
...
const socket = io(
    Object.keys(
        JSON.parse(fs.readFileSync(path.join(process.resourcesPath, 'customServer.json'), 'utf8')),
    ).length
        ? JSON.parse(fs.readFileSync(path.join(process.resourcesPath, 'customServer.json'), 'utf8'))
              .socketProtocol +
              '//' +
              JSON.parse(
                  fs.readFileSync(path.join(process.resourcesPath, 'customServer.json'), 'utf8'),
              ).socketHostname +
              (JSON.parse(
                  fs.readFileSync(path.join(process.resourcesPath, 'customServer.json'), 'utf8'),
              ).socketPort
                  ? ':' +
                    JSON.parse(
                        fs.readFileSync(
                            path.join(process.resourcesPath, 'customServer.json'),
                            'utf8',
                        ),
                    ).socketPort
                  : '')
...
const createWindow = () => {
    const window = new BrowserWindow({
        ...
        skipTaskbar:
            ...
            (fs.readFileSync(path.join(process.resourcesPath, 'autoLaunchType.txt'), 'utf8') ||
        ...
        autoUpdater.setFeedURL({
            ...{
                url:
                    ...
                    new URL(require('./package.json').repository).pathname +
                    '/' +
                    process.platform +
                    '-' +
                    process.arch +
                    '/' +
                    app.getVersion() +
                    (process.platform === 'darwin' ? '/RELEASES.json' : ''),
            },
    ...
    setTimeout(() => {
                    ...
                    [roomId]: setInterval(() => {
                        try {
                            ...
                            setTimeout(() => {
                                ...
                                socket.emit(
                                    'cpuUsage',
                                    (1 - (endIdle - startIdle) / (endTotal - startTotal)) * 100,
                                );
                                socket.emit('memoryUsage', (os.freemem() / os.totalmem()) * 100);
        ...
        socket.on('disconnected', (deviceId) => {
            if (systemUsageDataIntervals[deviceId])
                clearInterval(systemUsageDataIntervals[deviceId]);
        ...
        ipcMain.on('scriptError', (_, { language, err }) => {
            dialog.showErrorBox(
                'A ' +
                    ...
                    ' error occured in the renderer process',
                err,
            );
        });
