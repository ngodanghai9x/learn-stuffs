...
where sv.Masv = kq.Masv and mh.Mamh = kq.Mamh and mh.Mamh = 1 
	and Diem > (select Diem from Ketqua where Mamh = 1 and Masv = 3)
;
...
SELECT *
FROM sinhvien sv JOIN ketqua kq ON kq.Masv = sv.Masv
WHERE kq.Diem < 5;

SELECT sv.Lop, sv.Masv, sv.Tensv, COUNT(kq.Mamh)
FROM sinhvien sv JOIN ketqua kq ON kq.Masv = sv.Masv
WHERE kq.Diem < 5
GROUP BY sv.Lop, sv.Masv, sv.Tensv
HAVING COUNT(kq.Mamh) > 1;
